...
for s in susc_list:
	...
	Itstep_file = 'Results/Itstep_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.txt' %(numsims, b, s)
	Rtstep_file = 'Results/Rtstep_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.txt' %(numsims, b, s)
...
plt.xlabel('child susceptibility coefficient (epidemics only)')
plt.ylabel('Attack Rate')
lines = [CH, AD, TO, SR]
plt.legend(lines, ['children (5-18)', 'adults (19-64)', 'toddlers (0-2)', 'seniors (65+)'], loc = 'upper left')
plt.xlim([1, 1.5])
plt.ylim([0, 1])
figname = 'Figures/HR-AR_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.png' %(numsims, b, s)
plt.savefig(figname)
plt.clf()
...
plt.ylim([0, 10])
plt.xlim([1, 1.5])
figname = 'Figures/totepiOR_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.png' %(numsims, b, s)
plt.savefig(figname)
plt.clf()
pp.compress_to_ziparchive(zipname, figname)
# plt.show()
...
plt.ylim([0, 10])
plt.xlim([1, 1.5])
figname = 'Figures/totepiOR-avgs_childsusc_time_%ssims_beta%.3fsusc%.1f_vax0.png' %(numsims, b, s)
plt.savefig(figname)
plt.clf()
pp.compress_to_ziparchive(zipname, figname)
# plt.show()
...
for s in susc_epi:
	...
	plt.ylim([0, 20])
	plt.xlim([-1, 150])
	figname = 'Figures/epiORalign_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.png' %(numsims, b, s)
	plt.savefig(figname)
	plt.clf()
	pp.compress_to_ziparchive(zipname, figname)
...
for s in susc_epi:
	...
	# save plot
	figname = 'Figures/epiORincid_childsusc_time_%ssims_beta%.3f_susc%.1f_vax0.png' %(numsims, b, s)
	plt.savefig(figname)
	plt.clf()
	pp.compress_to_ziparchive(zipname, figname)
