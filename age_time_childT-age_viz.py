...
###Date: 6/2/14
###Purpose: visualize results of time-based epidemic simulations with varying T values by age
#### pairs with age_time_childT-age.py

###Import data: 

###Command Line: python age_time_childT-age_viz.py
##############################################
...
zipname = '/home/elee/Dropbox/Elizabeth_Bansal_Lab/Age_Based_Simulations/Results/childT-age_time_%ssims_beta%.3f_Tmult%.1f-%.1f_vax0.zip' %(numsims, b, m1, m2)
...
for m in Tmult_list:
	...
	Itstep_file = 'Results/Itstep_childT-age_time_%ssims_beta%.3f_Tmult%.1f_vax0.txt' %(numsims, b, m)
	Rtstep_file = 'Results/Rtstep_childT-age_time_%ssims_beta%.3f_Tmult%.1f_vax0.txt' %(numsims, b, m)
...
for m in Tmult_epi:
	...
	plt.xlabel('time step, child T multiplier: ' + str(m))
	plt.ylabel('OR, child:adult')
	plt.ylim([0, 30])
	plt.xlim([-1, 200])
	figname = 'Figures/epiOR_childT-age_time_%ssims_beta%.3f_Tmult%.1f_vax0.png' %(numsims, b, m)
	plt.savefig(figname)
	plt.close()
...
for m in Tmult_epi:
	...
	plt.plot(xrange(250), [1] * 250, marker = 'None', color = 'red', linewidth = 2)
	plt.xlabel('sim time step, child T multiplier: ' + str(m) + ', 5-95% cum infections')
	plt.ylabel('OR, child:adult')
	plt.ylim([0, 30])
	plt.xlim([-1, 200])
	figname = 'Figures/epiORfilt_childT-age_time_%ssims_beta%.3f_Tmult%.1f_vax0.png' %(numsims, b, m)
	plt.savefig(figname)
	plt.close()
	pp.compress_to_ziparchive(zipname, figname)
# 	plt.show()
...
for m in Tmult_epi:
	...
	plt.ylim([0, 30])
	plt.xlim([-1, 200])
figname = 'Figures/epiORfilt_childT-age_time_%ssims_beta%.3f_allTmult_vax0.png' %(numsims, b)
plt.savefig(figname)
plt.close()
pp.compress_to_ziparchive(zipname, figname)
# plt.show()
...
for m in Tmult_epi:
	...
	plt.xlim([-1, 200])
	figname = 'Figures/epiincid_childT-age_time_%ssims_beta%.3f_Tmult%.1f_vax0.png' %(numsims, b, m)
	plt.savefig(figname)
	plt.close()
	pp.compress_to_ziparchive(zipname, figname)
# 	plt.show()
...
figname = 'Figures/episize_childT-age_time_%ssims_beta%.3f_vax0.png' %(numsims, b)
plt.savefig(figname)
plt.close()
pp.compress_to_ziparchive(zipname, figname)
# plt.show()
