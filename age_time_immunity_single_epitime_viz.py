...
for imm in imm_epi:
	...
	for k0, k1, t5 in zip((k[0] for k in dummyk), (k[1] for k in dummyk), d_dummyalign_tstep[imm]):
	...
	# save plot
	figname = 'Figures/epiORincid_immunity_time_%ssims_beta%.3f_%s_%s.png' %(numsims, b, zstring, zstring3)
	plt.savefig(figname)
	plt.clf()
	pp.compress_to_ziparchive(zipname, figname)
	print "ORincid plotting time", imm, clock() - ORincid
	# plt.show()
