...
###Date: 7/15/14
###Function: Conduct time-based simulations on age-structure networks where pre-existing immunity exists and is heterogeneous within the adult population. Vary proportion of adults that have any pre-existing immunity. Choose a single immunity magnitude value.

###Import data: 

###Command Line: python age_time_immunity_prop.py
##############################################
...
imm_val = par.sp_immune_val
prop_ls = par.sp_prop_list
zstring = par.sp_pstr_range
zstring2 = par.sp_mstr_fixed
...
for prop in prop_ls:
	...
	filename = 'Results/Itstep_immunity_time_%ssims_beta%.3f_%s_%s.txt' %(numsims, b, zstring3, zstring2)
	pp.print_sorteddlist_to_file(d_save_I_tstep, filename, numsims)
	pp.compress_to_ziparchive(zipname, filename)

	filename = 'Results/Rtstep_immunity_time_%ssims_beta%.3f_%s_%s.txt' %(numsims, b, zstring3, zstring2)
	pp.print_sorteddlist_to_file(d_save_R_tstep, filename, numsims)
	pp.compress_to_ziparchive(zipname, filename)

	print "total time for sims set:", clock() - totaltime
	print "Params:", numsims, size_epi, inf_period, g, T, b, imm_val, prop

print "total time for sims:", clock() - totalsimtime
print "age_time_immunity_prop.py complete"